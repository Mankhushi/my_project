...
namespace MSINS_API.Repositories.Implementation
{
    public class LeadershipRepository : ILeadershipRepository
    {
        ...
        public LeadershipRepository(IHttpContextAccessor httpContextAccessor, IOptions<BaseUrlSettings> baseUrlSettings, IConfiguration configuration, IMapper mapper)
        {
        ...
        public async Task<(int Code, string Message)> AddOrUpdateLeadershipAsync(LeadershipRequest leaderDto, string? fileUrl)
        {
            int statusCode = 500;
            ...
            using (var connection = new SqlConnection(_connectionString))
            {
                ...
                using (var command = new SqlCommand("AddOrUpdateLeadership", connection))
                {
                    command.CommandType = CommandType.StoredProcedure;

                    command.Parameters.AddWithValue("@LeadershipId", leaderDto.LeadershipId);
                    command.Parameters.AddWithValue("@LeaderName", leaderDto.LeaderName);
                    command.Parameters.AddWithValue("@Designation", leaderDto.Designation);
                    command.Parameters.AddWithValue("@ImagePath", (object?)fileUrl ?? DBNull.Value);
                    command.Parameters.AddWithValue("@IsActive", leaderDto.IsActive);
                    command.Parameters.AddWithValue("@UserId", leaderDto.adminId);
        ...
        {
            var leaders = new List<LeadershipResponse>();
            using (var connection = new SqlConnection(_connectionString))
            {
                await connection.OpenAsync();

                using (var command = new SqlCommand("GetAllLeadership", connection))
                {
                    command.CommandType = CommandType.StoredProcedure;
                    command.Parameters.AddWithValue("@LeaderName", (object?)queryParams.LeaderName ?? DBNull.Value);
                    command.Parameters.AddWithValue("@Designation", (object?)queryParams.Designation ?? DBNull.Value);
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        ...
                        while (await reader.ReadAsync())
                        {
                            ...
                            var recordDetail = _mapper.Map<LeadershipResponse>(reader);
                            ...
                            leaders.Add(recordDetail); // Add the mapped recruiter to the list
                    ...
                    return new PagedResponse<LeadershipResponse>
                    {
                        Data = leaders,
