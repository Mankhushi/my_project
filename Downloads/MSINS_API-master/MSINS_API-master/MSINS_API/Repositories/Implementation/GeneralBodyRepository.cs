...
{
    public class GeneralBodyRepository: IGeneralBodyRepository
    {
        ...
        public GeneralBodyRepository(IHttpContextAccessor httpContextAccessor, IOptions<BaseUrlSettings> baseUrlSettings, IConfiguration configuration, IMapper mapper)
        {
        ...
        public async Task<(int Code, string Message)> AddOrUpdateGeneralBodyAsync(GeneralBodyRequest councilDto)
        {
            int statusCode = 500;
            ...
            using (var connection = new SqlConnection(_connectionString))
            {
                ...
                using (var command = new SqlCommand("AddOrUpdateGeneralBody", connection))
                {
                    command.CommandType = CommandType.StoredProcedure;

                    command.Parameters.AddWithValue("@GeneralBodyId", councilDto.GeneralBodyId);
        ...
        {
            var council = new List<GeneralBodyResponse>();
            using (var connection = new SqlConnection(_connectionString))
            {
                await connection.OpenAsync();

                using (var command = new SqlCommand("GetAllGeneralBody", connection))
                {
                    command.CommandType = CommandType.StoredProcedure;
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        ...
                        while (await reader.ReadAsync())
                        {
                            ...
                            var recordDetail = _mapper.Map<GeneralBodyResponse>(reader);
                    ...
                    return new PagedResponse<GeneralBodyResponse>
                    {
                        Data = council,
